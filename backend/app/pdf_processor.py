...
class PDFProcessor:
    ...
    def _extract_page1(self, result) -> Dict[str, Any]:
        ...
        Contains: Header, Section 1 (enrollment), Section 2 (daily frequency/meals for 31 days)
        """
        ...
        page1_tables = [table for table in result.tables
                        if table.bounding_regions and table.bounding_regions[0].page_number == 1]

        logger.info(f"Found {len(page1_tables)} tables on page 1")
        for i, table in enumerate(page1_tables):
            logger.info(f"  Table {i+1}: rows={table.row_count}, cols={table.column_count}")

        # Identify Section 1 and Section 2 tables
        section1_table = None
        section2_table = None
        ...
        for table in page1_tables:
            if table.row_count <= 6:  # Section 1 is small (4-5 rows + header + total)
                ...
                logger.info(f"Identified Section 1 table: rows={table.row_count}, cols={table.column_count}")
            elif table.row_count >= 30 and table.column_count >= 30:  # Section 2 is large: ~35 rows, 36 columns
                section2_table = self._parse_table(table, page_number=1, section="section2")
                logger.info(f"Identified Section 2 table: rows={table.row_count}, cols={table.column_count}")
    ...
    def _extract_page2(self, result) -> Dict[str, Any]:
        ...
        Contains: Section 3 (continuation of grid/attendance data)
        """
        ...
        logger.info(f"Found {len(page2_tables)} tables on page 2")
        for i, table in enumerate(page2_tables):
            logger.info(f"  Table {i+1}: rows={table.row_count}, cols={table.column_count}")

        # Identify Section 3 continuation (if present)
        section3_page2 = None

        for i, table in enumerate(page2_tables):
            # Section 3: Special diet data (31 days + 2 headers + total = ~34 rows, 12 columns)
            if table.row_count >= 30 and table.row_count <= 35 and table.column_count >= 10:
                ...
                logger.info(f"Identified Section 3 table: rows={table.row_count}, cols={table.column_count}")
                break
    ...
    def _parse_table(self, table, page_number: int, section: str) -> PDFTable:
        ...
        """
        # Debug: Show first few cells from Azure DI
        if section == "section2":
            print(f"\nDEBUG: Azure DI raw table cells (first 10 from Section 2):")
            for i, cell in enumerate(table.cells[:10]):
                print(f"  Cell [{cell.row_index},{cell.column_index}]: '{cell.content}' (span: {cell.row_span}x{cell.column_span})")
        ...
        # Debug: Show what the parsed 2D array looks like
        if section == "section2":
            print(f"\nDEBUG: Parsed table structure:")
            print(f"  Total rows: {len(cells)}")
            print(f"  Total columns: {len(cells[0]) if cells else 0}")
            print(f"\n  First 3 rows (showing first 12 columns):")
            for row_idx in range(min(3, len(cells))):
                row_data = cells[row_idx][:12] if row_idx < len(cells) else []
                print(f"    Row {row_idx}: {row_data}")
            if len(cells) > 3:
                print(f"\n  Data row (row 3, day 1) - first 12 columns:")
                print(f"    {cells[3][:12]}")
    ...
    def _stitch_pages(
        ...
        elif page2_data.get("section3_page2"):
            # Section 3 is only on page 2
            section3_combined = page2_data["section3_page2"]
        
        # Calculate overall confidence
        confidences = []
        for table_key in ["section1_table", "section2_table"]:
            if page1_data.get(table_key):
                confidences.append(page1_data[table_key].confidence)
        for table_key in ["section3_page2"]:
        ...
        return PDFReconciliationData(
            ...
            section2_table=page1_data.get("section2_table"),  # Section 2 is on page 1!
